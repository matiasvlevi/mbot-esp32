...
{
    ESP_ERROR_CHECK(mcpwm_gpio_init(MCPWM_UNIT_0, MCPWM0A, MOTOR_RIGHT_DDR));
...
{
    gpio_set_level(MOTOR_RIGHT_DDR, right);
}

void mbot_motor_set_dir_left(bool left) 
{
    gpio_set_level(MOTOR_LEFT_DDR, left);
}
...
{
    gpio_reset_pin(MOTOR_RIGHT_DDR);
    gpio_set_direction(MOTOR_RIGHT_DDR, GPIO_MODE_OUTPUT);

    gpio_reset_pin(MOTOR_LEFT_DDR);
    gpio_set_direction(MOTOR_LEFT_DDR, GPIO_MODE_OUTPUT);

    gpio_set_level(MOTOR_RIGHT_DDR, 0x01);
    gpio_set_level(MOTOR_LEFT_DDR,  0x01);
}
