...
export async function fetchNode(blueprints, nodes, edges, port, store, abortController) {
 ...
 * @returns
 */
export async function execTriggeredNodes(blueprints, nodes, edges, node, dispatchNode, store, setStoreItem, abortController, eventInstance) {
    if (abortController.signal.aborted) {
        return true;
    }
    ;
    ...
    if (bp.Name === "Event") {
        ...
        if (port)
            port.Cache = FormatEvent(eventInstance);
    }
    /**
...
export function findEventNodes(nodes, name) {
...
export async function exec(blueprints, nodes, edges, dispatchEdge, dispatchNode, store, eventStore, setStoreItem, setEventStoreItem, abortController) {
    if (abortController.signal.aborted)
        return true;
    // Clear Cache
    ...
    const findEventsAndTrigger = async (name, eventInstance) => {
        ...
        const port = EventNode.ports.find((p) => p.type === 'trigger' && p.role === 'output');
        if (!port)
